                            ...
                            <table class="table">
                                ...
                                <tbody>
                                    @forelse ($all_adds as $all_adds)
                                        <tr>
                                                <th>{{ $loop->index+1 }}</th>
                                                <td>{{ $all_adds->caption1 }}</td>
                                                <td>{{ $all_adds->caption2 }}</td>
                                                <td>{{ $all_adds->serialno }}</td>
                                                <td>
                                                    <img src="{{ asset($all_adds->photo) }}" alt="not found">
                                                </td>
                                            <td>
                                                <div class="div" style="margin-top:-2px">
                                                    @if ($all_adds->status == 1)
                                                        <a href="{{ route('slider_status',$all_adds->id) }}"><i class="fa fa-toggle-on" style="font-size: 24px"></i></a>
                                                    @else
                                                        <a href="{{ route('slider_status',$all_adds->id) }}"><i class="fa fa-toggle-off" style="font-size: 24px"></i></a>
                                                    @endif
                                                </div>
                                            </td>
